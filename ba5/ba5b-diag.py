...
def InputParser(file_path: str) -> dict:
    ...
    with open(file_path, 'r') as file:
        ...
        while True:
            ...
            if not line:
                params['diag'] = pd.DataFrame(mat)
                break

            # '-' separates down weight matrix and right weight matrix
            if '-' in line:
                # sep_flag == 1 means it had been already hit '-' before
                if sep_flag:
...
'''
def ManhattanTourist(n: int, m: int, Down: pd.DataFrame, Right: pd.DataFrame, Diag: pd.DataFrame) -> int:
    ...
    for i in range(1, m+1):
        for j in range(1, n+1):
            mat[i][j] = max([mat[i-1][j] + Right[i-1][j], mat[i][j-1] + Down[i][j-1], mat[i-1][j-1] + Diag[i-1][j-1]])
...
if __name__ == '__main__':
    ...
    print(ManhattanTourist(params['n'], params['m'], params['down'], params['right'], params['diag']))
